...
module.exports = {test: async function (provider, testingContext) {
  ...
  // obtener direcciones disponibles y asignarlas a varios roles
  const addresses = await web3.eth.getAccounts()
  if (addresses.length < 1) {
    console.log('no se pueden encontrar suficientes direcciones para ejecutar pruebas')
    process.exit(1)
  }
  ...
  async function runTest(
    ...
    if (callOrSend === 'send') {
      ...
      console.error('debe usar llamar o enviar')
      process.exit(1)
    }
  ...
  async function setupNewDefaultAddress(newPrivateKey) {
    ...
    if (txCount > 0) {
      console.warn(
        `warning: ${pubKey.address} ya ha sido utilizada, lo que puede causar ` +
        'que algunas pruebas fallen.'
      )
    }
  ...
  async function raiseGasLimit(necessaryGas) {
    ...
    if (necessaryGas > 8000000) {
      console.error('El nivel de gas necesario es demasiado alto')
      process.exit(1)
    ...
    // mostrar el límite de gasolina si es necesario haciendo transacciones adicionales
  ...
  async function getDeployGas(dataPayload) {
    await web3.eth.estimateGas({
      ...
      if (
        error.message === (
          'Error devuelto: el gas requerido excede lo permitido o siempre falla ' +
          'transaction'
        )
      ) {
        await raiseGasLimit()
